...
void print_runtime_result(
    ...
    const std::vector<std::pair<std::string, dtessl::Event>>& injections,
    bool color) {
  static constexpr std::string_view green = "\033[1;32m";
  static constexpr std::string_view cyan = "\033[1;36m";
  static constexpr std::string_view magenta = "\033[1;35m";
  ...
  for (const auto& [procedure, event] : injections) {
    std::cout << "│ " << styled(procedure, magenta, color) << "  ←  "
              << styled(event_text(event), cyan, color) << '\n';
  }
...
void repl_help() {
  std::cout
      ...
      << "  :inject P EVENT [...]    inject typed context; use -- for same RoundId\n"
      ...
      << "  :quit                    exit\n"
      << "\ncore v0.3.0 syntax:\n"
      << "  procedure/inject          persistent automaton + typed context admission\n"
      << "  replay/capture closed     context replay + complete procedure closure\n"
  ...
  while (true) {
    ...
    try {
      ...
      if (command == ":help") {
        ...
        std::vector<std::pair<std::string, dtessl::Event>> injections;
        std::size_t cursor = 1;
        while (cursor < words.size()) {
          ...
          injections.emplace_back(
              procedure, parse_event(words, event_start, cursor));
