...
| `v0.3.0` | Composite state and native trace | Orthogonal `@context` axes, `case (source-set)->(target-set)`, exact/set/wildcard source patterns, compact contextual updates, path-local guard/ActionPlan, isolated persistent procedure instances in a language RuntimeContext, causal-DAG RoundId, procedure-aware replay/capture filters and finite-prefix claims |
